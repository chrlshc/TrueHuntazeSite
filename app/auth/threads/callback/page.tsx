    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setStatus('success');
          setMessage('Threads connected successfully!');
          setTimeout(() => router.push('/platforms/connect'), 2000);
        } else {
          const data = await response.json();
          setStatus('error');
          setMessage(data.error || 'Failed to connect Threads');
        }
      } catch (err) {
        setStatus('error');
        ...
        <div className="text-center">
          {status === 'loading' && (
            <>
              ...
              <p className="text-gray-600">Connecting to Threads...</p>
            </>
          )}
          ...
          {status === 'error' && (
            <>
              ...
              <button
                onClick={() => router.push('/auth/threads')}
